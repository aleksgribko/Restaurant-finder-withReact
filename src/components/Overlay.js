...
class Overlay extends Component {      
	...
	reviewListNew(){
		console.log('state in Overlay', this.state.reviewList)
		...
		return (
			<div className="overlay" id={this.props.name}>
				<div id='inOverlay'>	
					...
					<div className='overLayLeft' style={{  textAlign: 'center', marginTop:"10px", flexGrow: '1'}}>								
						<h2 onClick={this.off}>{this.props.name}</h2>
						<p>address: {this.props.address}</p>						
						<img className='img' alt='streetview' style={{ height:'400px'}}></img>	
					</div>
											
						<div className='overLayRight' style={{ height:'90vh', marginTop:"10px", width: '25%', flexGrow: '1'}}>							
							
							<h3>Average ranking: {this.props.rating}</h3>
							{this.reviewListNew()}
						</div>	
					
					<div style={{ height:'90vh', marginTop:"10px", width: '25%', flexGrow: '1'}}>
						<button style={{height:"50px", width:'100%', marginTop:"10px" }} onClick={this.off}>	
						Back to the map 
						</button>	
						<FormInOverlay reviews={this.props.reviews} storeNewReview={this.storeNewReview}/>
					</div>
				</div>
			</div>
		)	
