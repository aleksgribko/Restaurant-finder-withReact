...
class RestoOnMap extends Component {
	...
	switchOverlay() {		//hide all overlays
		...
		fetch(`https://maps.googleapis.com/maps/api/streetview?size=400x400&location=${this.props.lat},${this.props.lng}&fov=90&heading=235&pitch=10&key=AIzaSyDCJD8ghgxEJJdmUIr9_m0mY_wBEUOW5Dw`) // input key here for streetview
	...
	render(){	
		...
		<div 
			id={this.props.id}
			style={{width:this.props.size*2, height:this.props.size*2, position: 'absolute',  transform: 'translate(-50%, -50%)'}}
			name={this.props.name}  
			lat={this.props.lat}
	      	lng={this.props.lng}
	      	className={this.props.type === 'featured' ? 'pointerRestoFeatured' : 'pointerResto'}
